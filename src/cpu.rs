...
impl CPU {
    ...
    fn cpi(&mut self) {
        let bc = self.reg.get_bc();
        let hl = self.reg.get_hl();
        let h = self.bus.borrow().read_byte(hl);
    ...
    fn bit(&mut self, operand: u8) {
        ...
        let r = match register {
            ...
            6 => bit::get(self.bus.borrow().read_byte(self.reg.get_hl()), bit),
    ...
    fn call_stack_push(&mut self) {
        ...
        self.bus
            .borrow_mut()
            .write_word(self.reg.sp, self.reg.pc.wrapping_add(3));
    }

    // call stack pop
    fn call_stack_pop(&mut self) {
        self.reg.pc = self.bus.borrow().read_word(self.reg.sp);
    ...
    fn interrupt_stack_push(&mut self) {
        self.reg.sp = self.reg.sp.wrapping_sub(2);
        self.bus.borrow_mut().write_word(self.reg.sp, self.reg.pc);
    }
    ...
    fn ini(&mut self) {
        ...
        let hl = self.reg.get_hl();
        self.bus.borrow_mut().write_byte(hl, d);
    ...
    fn get_io(&mut self, port: u8) -> u8 {
        if let Ok(_) = self.bus.borrow_mut().io_req.0.send_timeout(port, Duration::from_millis(16)) {
    ...
    fn set_io(&mut self, port: u8, data: u8) {
        ...
        if let Err(_) =  self.bus.borrow_mut().io_out.0.send_timeout((port,data), Duration::from_millis(16)) {
    ...
    fn execute_4bytes(&mut self) -> u32 {
        let opcode = self.bus.borrow().read_le_dword(self.reg.pc);
        let cycles;

        match opcode & 0xFFFF00FF {
            0xDDCB0006 => {
                // RLC (IX+d)
                let displacement = self.bus.borrow().read_byte(self.reg.pc + 2);
                if bit::get(displacement, 7) {
                    let m = self.reg.get_ix() - (signed_to_abs(displacement) as u16);
                    let d = self.bus.borrow().read_byte(m);
                    let r = self.rlc(d);
                    self.bus.borrow_mut().write_byte(m, r);
                } else {
            ...
            | 0xDDCB0076 | 0xDDCB007E => {
                // BIT b,(IX+d)
                let displacement = self.bus.borrow().read_byte(self.reg.pc + 2);
                let operand = self.bus.borrow().read_byte(self.reg.pc + 3);
                let bit = ((operand & 0x38) >> 3) as usize;
                if bit::get(displacement, 7) {
                    let m = self.reg.get_ix() - (signed_to_abs(displacement) as u16);
                    let d = self.bus.borrow().read_byte(m);
    ...
    fn execute_2bytes(&mut self) -> u32 {
        let opcode = self.bus.borrow().read_le_word(self.reg.pc);
        ...
        match opcode {
            ...
            0xDD46 => {
                ...
                if bit::get(displacement, 7) {
                    self.reg.b = self
                        .bus
                        .borrow()
                        .read_byte(self.reg.get_ix() - (signed_to_abs(displacement) as u16))
                } else {
                    self.reg.b = self
                        .bus
                        .borrow()
                        .read_byte(self.reg.get_ix() + (displacement as u16))
                }
            }
            0xDD4E => {
                // LD C,(IX+d)
            ...
            0xFD46 => {
                ...
                if bit::get(displacement, 7) {
                    self.reg.b = self
                        .bus
                        .borrow()
                        .read_byte(self.reg.get_iy() - (signed_to_abs(displacement) as u16))
                } else {
                    self.reg.b = self
                        .bus
                        .borrow()
                        .read_byte(self.reg.get_iy() + (displacement as u16))
                }
            }
            0xFD4E => {
                // LD C,(IY+d)
            ...
            0xDD70 => {
                ...
                let displacement = self.bus.borrow().read_byte(self.reg.pc + 2);
                if bit::get(displacement, 7) {
                    self.bus.borrow_mut().write_byte(
                        self.reg.get_ix() - (signed_to_abs(displacement) as u16),
                        self.reg.b,
                    )
                } else {
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), self.reg.b)
                }
            }
            0xDD71 => {
                // LD (IX+d),C
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), self.reg.c)
                }
            }
            0xDD72 => {
                // LD (IX+d),D
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), self.reg.d)
                }
            }
            0xDD73 => {
                // LD (IX+d),E
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), self.reg.e)
                }
            }
            0xDD74 => {
                // LD (IX+d),H
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), self.reg.h)
                }
            }
            0xDD75 => {
                // LD (IX+d),L
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), self.reg.l)
                }
            }
            0xDD77 => {
                // LD (IX+d),A
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), self.reg.a)
                }
            }
            ...
            0xFD70 => {
                ...
                if bit::get(displacement, 7) {
                    self.bus.borrow_mut().write_byte(
                        self.reg.get_iy() - (signed_to_abs(displacement) as u16),
                        self.reg.b,
                    )
                } else {
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), self.reg.b)
                }
            }
            0xFD71 => {
                // LD (IY+d),C
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), self.reg.c)
                }
            }
            0xFD72 => {
                // LD (IY+d),D
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), self.reg.d)
                }
            }
            0xFD73 => {
                // LD (IY+d),E
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), self.reg.e)
                }
            }
            0xFD74 => {
                // LD (IY+d),H
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), self.reg.h)
                }
            }
            0xFD75 => {
                // LD (IY+d),L
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), self.reg.l)
                }
            }
            0xFD77 => {
                // LD (IY+d),A
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), self.reg.a)
                }
            }
            ...
            0xDD36 => {
                let displacement = self.bus.borrow().read_byte(self.reg.pc + 2);
                let data = self.bus.borrow().read_byte(self.reg.pc + 3);
                if bit::get(displacement, 7) {
                    self.bus.borrow_mut().write_byte(
                        self.reg.get_ix() - (signed_to_abs(displacement) as u16),
                        data,
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_ix() + (displacement as u16), data)
                }
            }
            ...
            0xDD21 => {
                self.reg
                    .set_ix(self.bus.borrow().read_word(self.reg.pc + 2));
            }

            // LD IY,nn
            0xFD21 => {
                self.reg
                    .set_iy(self.bus.borrow().read_word(self.reg.pc + 2));
            }
            ...
            0xFD36 => {
                ...
                if bit::get(displacement, 7) {
                    ...
                    self.bus
                        .borrow_mut()
                        .write_byte(self.reg.get_iy() + (displacement as u16), data)
                }
            }
            ...
            0xED4B => {
                // LD BC,(nn)
                let addr = self.bus.borrow().read_word(self.reg.pc + 2);
                let d = self.bus.borrow().read_word(addr);
                self.reg.set_bc(d);
            }
            ...
            0xED43 => {
                // LD (nn),BC
                let addr = self.bus.borrow().read_word(self.reg.pc + 2);
                self.bus.borrow_mut().write_word(addr, self.reg.get_bc());
            }

            0xED53 => {
                // LD (nn),DE
                let addr = self.bus.borrow().read_word(self.reg.pc + 2);
                self.bus.borrow_mut().write_word(addr, self.reg.get_de());
            }

            0xED63 => {
                // LD (nn),HL
                let addr = self.bus.borrow().read_word(self.reg.pc + 2);
                self.bus.borrow_mut().write_word(addr, self.reg.get_hl());
            }

            0xED73 => {
                // LD (nn),SP
                let addr = self.bus.borrow().read_word(self.reg.pc + 2);
                self.bus.borrow_mut().write_word(addr, self.reg.sp);
            }

            // LD (nn),IX
            0xDD22 => {
                let addr = self.bus.borrow().read_word(self.reg.pc + 2);
                self.bus.borrow_mut().write_word(addr, self.reg.get_ix());
            }

            // LD (nn),IY
            0xFD22 => {
                let addr = self.bus.borrow().read_word(self.reg.pc + 2);
                self.bus.borrow_mut().write_word(addr, self.reg.get_iy());
            }
            ...
            0xDDE5 => {
                ...
                self.bus
                    .borrow_mut()
                    .write_word(self.reg.sp, self.reg.get_ix());
            }
            ...
            0xFDE5 => {
                ...
                self.bus
                    .borrow_mut()
                    .write_word(self.reg.sp, self.reg.get_iy());
            }

            // POP IX
            0xDDE1 => {
                self.reg.set_ix(self.bus.borrow().read_word(self.reg.sp));
                self.reg.sp = self.reg.sp.wrapping_add(2);
            }

            // POP IY
            0xFDE1 => {
                self.reg.set_iy(self.bus.borrow().read_word(self.reg.sp));
                self.reg.sp = self.reg.sp.wrapping_add(2);
            }
            ...
            // EX (SP),IX
            0xDDE3 => {
                let pointed_by_sp = self.bus.borrow().read_word(self.reg.sp);
                self.bus
                    .borrow_mut()
                    .write_word(self.reg.sp, self.reg.get_ix());
            ...
            0xCB06 => {
                ...
                let d = self.bus.borrow().read_byte(addr);
                let r = self.rlc(d);
                self.bus.borrow_mut().write_byte(addr, r);
            }
            ...
            // RLD
            0xED6F => {
                let hl_contents = self.bus.borrow().read_byte(self.reg.get_hl());
            ...
            // BIT b,r
            0xCB40..=0xCB7F => {
                let b = self.bus.borrow().read_byte(self.reg.pc + 1);
                self.bit(b);
            }
            ...
            // LD IXH,n
            0xDD26 => {
                let n = self.bus.borrow().read_byte(self.reg.pc + 3);
                self.reg.ixh = n;
            }
    ...
    fn execute_1byte(&mut self, opcode: u8) -> u32 {
        ...
        match opcode {
            ...
            0x46 => {
                // LD B,(HL)
                let addr = self.reg.get_hl();
                self.reg.b = self.bus.borrow().read_byte(addr)
            }
            ...
            0x4E => {
                // LD C,(HL)
                let addr = self.reg.get_hl();
                self.reg.c = self.bus.borrow().read_byte(addr)
            }
            ...
            0x56 => {
                // LD D,(HL)
                let addr = self.reg.get_hl();
                self.reg.d = self.bus.borrow().read_byte(addr)
            }
            ...
            0x5E => {
                // LD E,(HL)
                let addr = self.reg.get_hl();
                self.reg.e = self.bus.borrow().read_byte(addr)
            }
            ...
            0x66 => {
                // LD H,(HL)
                let addr = self.reg.get_hl();
                self.reg.h = self.bus.borrow().read_byte(addr)
            }
            ...
            0x6E => {
                // LD L,(HL)
                let addr = self.reg.get_hl();
                self.reg.l = self.bus.borrow().read_byte(addr)
            }
            ...
            0x7E => {
                // LD A,(HL)
                let addr = self.reg.get_hl();
                self.reg.a = self.bus.borrow().read_byte(addr)
            }
            0x7F => {} // LD A,A
            ...
            0x70 => {
                // LD (HL), B
                let addr = self.reg.get_hl();
                self.bus.borrow_mut().write_byte(addr, self.reg.b)
            }
            0x71 => {
                // LD (HL), C
                let addr = self.reg.get_hl();
                self.bus.borrow_mut().write_byte(addr, self.reg.c)
            }
            0x72 => {
                // LD (HL), D
                let addr = self.reg.get_hl();
                self.bus.borrow_mut().write_byte(addr, self.reg.d)
            }
            0x73 => {
                // LD (HL), E
                let addr = self.reg.get_hl();
                self.bus.borrow_mut().write_byte(addr, self.reg.e)
            }
            0x74 => {
                // LD (HL), H
                let addr = self.reg.get_hl();
                self.bus.borrow_mut().write_byte(addr, self.reg.h)
            }
            0x75 => {
                // LD (HL), L
                let addr = self.reg.get_hl();
                self.bus.borrow_mut().write_byte(addr, self.reg.l)
            }

            0x77 => {
                // LD (HL), A
                let addr = self.reg.get_hl();
                self.bus.borrow_mut().write_byte(addr, self.reg.a)
            }

            // LD r,n
            0x06 => {
                // LD B,n
                let data = self.bus.borrow().read_byte(self.reg.pc + 1);
                self.reg.b = data;
            }
            0x0E => {
                // LD C,n
            ...
            // LD A,(nn)
            0x3A => {
                let addr = self.bus.borrow().read_word(self.reg.pc + 1);
                self.reg.a = self.bus.borrow().read_byte(addr);
            }
            ...
            0x01 => {
                // LD BC,nn
                let d16 = self.bus.borrow().read_word(self.reg.pc + 1);
                self.reg.set_bc(d16);
            }
            0x11 => {
                // LD DE,nn
            ...
            0x22 => {
                ...
                let addr = self.bus.borrow().read_word(self.reg.pc + 1);
                self.bus.borrow_mut().write_word(addr, d);
            }
            ...
            0xC5 => {
                ...
                self.bus
                    .borrow_mut()
                    .write_word(self.reg.sp, self.reg.get_bc());
            }
            0xD5 => {
                // PUSH DE
                self.reg.sp = self.reg.sp.wrapping_sub(2);
                self.bus
                    .borrow_mut()
                    .write_word(self.reg.sp, self.reg.get_de());
            }
            0xE5 => {
                // PUSH HL
                self.reg.sp = self.reg.sp.wrapping_sub(2);
                self.bus
                    .borrow_mut()
                    .write_word(self.reg.sp, self.reg.get_hl());
            }
            0xF5 => {
                // PUSH AF
                self.reg.sp = self.reg.sp.wrapping_sub(2);
                self.bus
                    .borrow_mut()
                    .write_byte(self.reg.sp, self.reg.flags.to_byte());
                self.bus
                    .borrow_mut()
                    .write_byte(self.reg.sp + 1, self.reg.a);
            }

            // POP qq
            0xC1 => {
                // POP BC
                self.reg.set_bc(self.bus.borrow().read_word(self.reg.sp));
                self.reg.sp = self.reg.sp.wrapping_add(2);
            }

            0xD1 => {
                // POP DE
                self.reg.set_de(self.bus.borrow().read_word(self.reg.sp));
                self.reg.sp = self.reg.sp.wrapping_add(2);
            }

            0xE1 => {
                // POP HL
                self.reg.set_hl(self.bus.borrow().read_word(self.reg.sp));
                self.reg.sp = self.reg.sp.wrapping_add(2);
            }
            ...
            0xE3 => {
                let pointed_by_sp = self.bus.borrow().read_word(self.reg.sp);
                let hl = self.reg.get_hl();
                self.bus.borrow_mut().write_word(self.reg.sp, hl);
            ...
            0x86 => {
                // ADD (HL)
                let addr = self.reg.get_hl();
                let n = self.bus.borrow().read_byte(addr);
                self.add(n)
            }
            ...
            // ADD A,n
            0xC6 => {
                let n = self.bus.borrow().read_byte(self.reg.pc + 1);
                self.add(n);
            }
            ...
            // JR e
            0x18 => {
                let displacement = self.bus.borrow().read_byte(self.reg.pc + 1);
                if bit::get(displacement, 7) {
                    self.reg.pc = self.reg.pc + 2 - (signed_to_abs(displacement) as u16)
                } else {
                    self.reg.pc = self.reg.pc + (displacement as u16) + 2
                }
            }
        ...
        if self.debug.opcode {
            print!("{:#06X}\t{}\nSP : {:#06X}\tS : {}\tZ : {}\tH : {}\tP : {}\tN : {}\tC : {}\nB : {:#04X}\tC : {:#04X}\tD : {:#04X}\tE : {:#04X}\tH : {:#04X}\tL : {:#04X}\tA : {:#04X}\t(SP) : {:#06X}\n", pc, self.dasm_1byte(pc), self.reg.sp, self.reg.flags.s as i32, self.reg.flags.z as i32, self.reg.flags.h as i32, self.reg.flags.p as i32, self.reg.flags.n as i32, self.reg.flags.c as i32, self.reg.b, self.reg.c, self.reg.d, self.reg.e, self.reg.h, self.reg.l, self.reg.a, self.bus.borrow().read_word(self.reg.sp))
        }
        ...
        match opcode {
            ...
            | 0xE6 | 0xF6 | 0xEE | 0xFE | 0xDB | 0xD3 => self.reg.pc += 2,
