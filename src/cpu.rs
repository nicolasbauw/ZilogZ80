...
const CYCLES: [u8; 256] = [
    ...
    5, 10, 10, 0, 10, 11, 7, 11, 5, 4, 10, 0, 10, 0, 7, 11,
    5, 10, 10, 19, 10, 11, 7, 11, 5, 4, 10, 4, 10, 0, 7, 11,
    5, 10, 10, 4, 10, 11, 7, 11, 5, 6, 10, 4, 10, 0, 7, 11,
];

const CYCLES_DD_FD: [u8; 256] = [
...
impl CPU {
    ...
    fn sbc(&mut self, n: u8)  {
        ...
        self.registers.flags.h = (a as i8 & 0x0f) < (n as i8 & 0x0f).wrapping_add(c as i8);
    ...
    fn rlca(&mut self) {
        self.registers.flags.c = bit::get(self.registers.a, 7);
        let r = (self.registers.a << 1) | u8::from(self.registers.flags.c);
        self.registers.flags.c = bit::get(self.registers.a, 7);
        self.registers.flags.h = false;
        self.registers.flags.n = false;
        self.registers.a = r;
    ...
    fn rrca(&mut self) {
        self.registers.flags.c = bit::get(self.registers.a, 0);
        let r = if self.registers.flags.c {0x80 | (self.registers.a >> 1) } else { self.registers.a >> 1 };
        self.registers.flags.h = false;
        self.registers.flags.n = false;
        self.registers.a = r;
    }
        ...
        match opcode & 0xFFFF00FF {
            ...
            0xFDCB0006 => {                                                           // RLC (IY+d)
                ...
                if bit::get(displacement, 7) {
                    ...
                    let m = self.registers.get_iy() + ( displacement as u16 );
                    let d = self.bus.read_byte(m);
                    let r = self.rlc(d);
                    self.bus.write_byte(m, r);
                }
        ...
        match opcode {
            ...
            0xDD86 => {
                ...
                if bit::get(displacement, 7) {
                    ...
                    let d = self.bus.read_byte(self.registers.get_ix() + ( displacement as u16 ));
                    self.add(d);
                }
            },
            ...
            0xFD86 => {
                ...
                if bit::get(displacement, 7) {
                    ...
                    let d = self.bus.read_byte(self.registers.get_iy() + ( displacement as u16 ));
                    self.add(d);
                }
            },
            ...
            0xDD23 => {                                                             // INC IX
                let r = self.registers.get_ix().wrapping_add(1);
                self.registers.set_ix(r);
            },

            0xFD23 => {                                                             // INC IY
                let r = self.registers.get_iy().wrapping_add(1);
                self.registers.set_iy(r);
            },

            0xDD2B => {                                                             // DEC IX
                let r = self.registers.get_ix().wrapping_sub(1);
                self.registers.set_ix(r);
            },

            0xFD2B => {                                                             // DEC IY
                let r = self.registers.get_iy().wrapping_sub(1);
                self.registers.set_iy(r);
            },
            ...
            // ADD A,IXH
            0xDD84 => {
                let n = self.registers.ixh;
                self.add(n);
            },

            // ADD A,IXL
            0xDD85 => {
                let n = self.registers.ixl;
                self.add(n);
            },

            // ADD A,IYH
            0xFD84 => {
                let n = self.registers.iyh;
                self.add(n);
            },

            // ADD A,IYL
            0xFD85 => {
                let n = self.registers.iyl;
                self.add(n);
            },
