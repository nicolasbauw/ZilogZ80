...
pub fn dasm<B: Bus + ?Sized>(bus: &B, address: u16) -> (String, u8) {
    ...
    let instr = match opcode {
        ...
        0xDD => {
            ...
            match opcode_16 {
                0xDD46 | 0xDD4E | 0xDD56 | 0xDD5E | 0xDD66 | 0xDD6E | 0xDD7E | 0xDD70 | 0xDD71
                | 0xDD72 | 0xDD73 | 0xDD74 | 0xDD75 | 0xDD77 | 0xDD86 | 0xDD8E | 0xDD96
                | 0xDD9E | 0xDDA6 | 0xDDB6 | 0xDDAE | 0xDDBE | 0xDD34 | 0xDD35 => {
                    let operand = bus.read_byte(address + 2);
                    format!("{:04X} {:02X}       {}", opcode_16, operand, dasm_str)
                }
                0xDD36 | 0xDD21 | 0xDD2A | 0xDD22 | 0xDDCB => {
                    let operand = bus.read_word(address + 2);
                    format!(
                        "{:04X} {:02X} {:02X}    {}",
                        opcode_16,
        ...
        0xFD => {
            ...
            match opcode_16 {
                0xFD46 | 0xFD4E | 0xFD56 | 0xFD5E | 0xFD66 | 0xFD6E | 0xDD7E | 0xFD7E | 0xFD70
                | 0xFD71 | 0xFD72 | 0xFD73 | 0xFD74 | 0xFD75 | 0xFD77 | 0xFD86 | 0xFD8E
                | 0xFD96 | 0xFD9E | 0xFDA6 | 0xFDB6 | 0xFDAE | 0xFDBE | 0xFD34 | 0xFD35 => {
                    let operand = bus.read_byte(address + 2);
                    format!("{:04X} {:02X}        {}", opcode_16, operand, dasm_str)
                }
                0xFD36 | 0xFD21 | 0xFD2A | 0xFD22 | 0xFDCB => {
                    let operand = bus.read_word(address + 2);
                    format!(
                        "{:04X} {:02X} {:02X}    {}",
                        opcode_16,
        ...
        // LD A,(nn)
        0x3A => {
            let addr_low = bus.read_byte(address + 1);
            let addr_high = bus.read_byte(address + 2);
            let addr = bus.read_word(address + 1);
            format!(
                "3A {:02X} {:02X}      LD A,(${:04X})",
        ...
        0x01 => {
            ...
            let addr_low = bus.read_byte(address + 1);
            let addr_high = bus.read_byte(address + 2);
            let d16 = bus.read_word(address + 1);
            format!(
        ...
        // JR e
        0x18 => {
            let displacement = bus.read_byte(address + 1);
        ...
        // IN A,(n)
        0xDB => {
            let port = bus.read_byte(address + 1);
    ...
    let instr_size_16: u8 = match opcode_16 {
        0xDD46 | 0xFD46 | 0xDD4E | 0xFD4E | 0xDD56 | 0xFD56 | 0xDD5E | 0xFD5E | 0xDD66 | 0xFD66
        | 0xDD6E | 0xFD6E | 0xDD7E | 0xFD7E | 0xDD70 | 0xDD71 | 0xDD72 | 0xDD73 | 0xDD74
        | 0xDD75 | 0xDD77 | 0xFD70 | 0xFD71 | 0xFD72 | 0xFD73 | 0xFD74 | 0xFD75 | 0xFD77
        | 0xDD86 | 0xFD86 | 0xDD8E | 0xFD8E | 0xDD96 | 0xFD96 | 0xDD9E | 0xFD9E | 0xDDA6
        | 0xFDA6 | 0xDDB6 | 0xFDB6 | 0xDDAE | 0xFDAE | 0xDDBE | 0xFDBE | 0xDD34 | 0xFD34
        | 0xDD35 | 0xFD35 => 3,
        0xDD36 | 0xFD36 | 0xDD21 | 0xFD21 | 0xED4B | 0xED5B | 0xED6B | 0xED7B | 0xDD2A | 0xFD2A
        | 0xED43 | 0xED53 | 0xED63 | 0xED73 | 0xDD22 | 0xFD22 | 0xDDCB | 0xFDCB => 4,
        _ => 2,
    };
    if opcode_16 == 0 {
