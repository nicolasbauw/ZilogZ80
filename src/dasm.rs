use crate::bit;
use crate::cpu::{signed_to_abs, CPU};

impl CPU {
    /// Disassembles code at (address)
    pub fn dasm_1byte(&self, address: u16) -> String {
        let opcode = self.bus.borrow().read_byte(address);
        match opcode {
            // 8-Bit Load Group
            // LD r,r'      LD r,(HL)
            ...
            0x06 => {
                // LD B,n
                let data = self.bus.borrow().read_byte(address + 1);
            ...
            // LD A,(nn)
            0x3A => {
                let addr_low = self.bus.borrow().read_byte(address + 1);
                let addr_high = self.bus.borrow().read_byte(address + 2);
                let addr = self.bus.borrow().read_word(address + 1);
                format!(
                    "3A {:02X} {:02X}      LD A,(${:04X})",
            ...
            0x01 => {
                ...
                let addr_low = self.bus.borrow().read_byte(address + 1);
                let addr_high = self.bus.borrow().read_byte(address + 2);
                let d16 = self.bus.borrow().read_word(address + 1);
                format!(
            ...
            // ADD A,n
            0xC6 => {
                let n = self.bus.borrow().read_byte(address + 1);
            ...
            // JR e
            0x18 => {
                let displacement = self.bus.borrow().read_byte(address + 1);
            ...
            // IN A,(n)
            0xDB => {
                let port = self.bus.borrow().read_byte(address + 1);
